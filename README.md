...
FE -> API: GET /api/form-schema/get/schema
API --> FE: 200 OK (JSON Form Schema)
FE -> FE: Render dynamic form

== Step 2: Submit Form ==
User -> FE: Fill form + Submit
FE -> FE: Disable submit button, show loader
FE -> API: POST /api/form-submissions/post/submission
API -> API: Validate with AJV
API -> DB: Store submission
DB --> API: OK
API --> FE: 201 Created (submissionId, createdAt)
FE -> FE: Show success message

== Step 3: View Submissions ==
User -> FE: Open submissions page
FE -> API: GET /api/get-submissions/get/submissions?page=1&sortBy=createdAt&sortOrder=desc
API -> DB: Query paginated + sorted results
DB --> API: JSON results
API --> FE: 200 OK (submissions)
FE -> FE: Render table
@enduml
Features
